  ...
  return (
    <View style={tw`bg-white flex-grow`}>
        <View>
            ...
            font-semibold text-lg`}>Select a ride</Text>
        </View>
        ...
        <FlatList 
            ...
            renderItem={({item: {id, title, image}, item})=> (
                <TouchableOpacity onPress={()=> setSelected(item)} 
                     >
                    <View style={tw`flex-row ${id === selected?.id && "bg-gray-200"} items-center justify-between px-10`}
                         >
        ...
        <View>
            <TouchableOpacity style={tw`m-3 bg-black py-5 flex items-center rounded-full ${!selected && "bg-gray-600"} `}>
